...
def buildDNN(Neuron_Distribution):
	...
	for i in xrange(len(Neuron_Distribution)-1):
		...
		w = theano.shared(np.random.normal(-.01, .01, (dim_next, dim_now)).astype(dtype='float32'))
		b = theano.shared(np.random.normal(-.01, .01, dim_next).astype(dtype='float32'))
	...
	mu_initial = theano.shared(np.float32(0.15))
