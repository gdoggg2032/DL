	...
	for(i=0; i<n; i++) { /*** example loop ***/
	  ...
	  if((!use_shrinking) || (opti[i] != opti_round)) {
	    ...
	    if(slack > (margin-dist+0.0001)) {
      	...
      	printf("margin=%lf, dist=%lf, slack=%lf, epsilon=%lf\n", margin, dist, slack, epsilon);
  ...
  */
  ...
  do { /* iteratively find and add constraints to working set */
	...
	/* exit(1); */
      ...
      if(struct_verbosity>=1)
	  	printf("(NumConst=%d, SV=%ld, CEps=%.4f, QPEps=%.4f, Loss=%.4f)\n",cset.m,
		 svmModel->sv_num-1,ceps,svmModel->maxdiff, rhs);
